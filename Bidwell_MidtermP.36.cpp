...
void displayRequirements() {
    ...
    cout << "The password must be at least " << MIN << " characters long,\n"
        ...
        << "have at least 1 digit, and have at least 1 special character.\n" << endl;
}
...
void displayResults(char str[]) {
    ...
    if (isValid(str)) {
        ...
        //Determine if the string contains a special character
        if (!hasSpecial(str)) {
...
bool hasSpecial(char str[]) {
    ...
    //Test each char until null is reached or special char is found
    while (*str != '\0' && count == 0) {
    ...
    //Determine if special chars counter is greater than 0
    if (count > 0) {
        //If so, status is true
        status = true;
    }
